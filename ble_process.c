...
/*uint8 scan_tag[SCAN_TAG_DATA_LEN] = {0x13, 0x21, 0xF0, 0x34, 0x9B, 0x5F, 0x80, 0x00,
...
//	CYBLE_GATTS_WRITE_CMD_REQ_PARAM_T	writeCmdData;
	...
	// CYBLE_GATTC_WRITE_REQ_T 			writeRIGHTdata;
	...
	switch(event)
	{
		...
		case CYBLE_EVT_GATTS_WRITE_REQ:
			...
			* connected Client device */
            ...
            if(writeReqData.handleValPair.attrHandle == CYBLE_MOTORSERVICE_SPEEDLEFT_CHAR_HANDLE)
            {
                speedLeft = writeReqData.handleValPair.value.val[0];
                RGBData[RGB_RED_INDEX] = speedLeft;
                
                #ifdef DEBUG_ENABLED
                UART_UartPutString("Speed Left");
                PrintNum(speedLeft);
                UART_UartPutCRLF(' ');
                #endif
            ...
            if(writeReqData.handleValPair.attrHandle == CYBLE_MOTORSERVICE_SPEEDRIGHT_CHAR_HANDLE)
            {  // setSpeed(RIGHT, (uint8)writeReqData.handleValPair.value.val[0]);
                speedRight = writeReqData.handleValPair.value.val[0];
                RGBData[RGB_GREEN_INDEX] = speedRight;
                #ifdef DEBUG_ENABLED
                UART_UartPutString("Speed Right");
                PrintNum(speedRight);
                UART_UartPutCRLF(' ');
                #endif
						...
						* marker, then the peripheral is a node of the network */
						if(scan_report.data[scan_report.dataLen-2] == CUSTOM_ADV_DATA_MARKER)
						{
							...
							* whose color has to be updated. */
							...
							((scan_report.data[scan_report.dataLen-1] == 100) && (dataADVCounter == 0)))
							{
		...
		case CYBLE_EVT_GAP_DEVICE_CONNECTED:
			...
			if(ble_gap_state == BLE_CENTRAL)
			{
					...
					* RGB LED control data using attribute handles */
                    ...
                    #ifdef DEBUG_ENABLED
						UART_UartPutString("----- ADVcounterdata ");
						PrintHex(writeADVcounterdata.value.val);
						UART_UartPutCRLF(' ');
                        
                        UART_UartPutString("----- writeRGBdata ");
						PrintHex(writeRGBdata.value.val);
						UART_UartPutCRLF(' ');
